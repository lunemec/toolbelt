...
## [Unreleased]
### Fixed
- Claude `~/.claude/` mount changed from read-write direct-mount to read-only secret-mount with hard-copy hydration into the container, matching all other tool config mounts. Container-side plugin installs, session state, and credential changes are now ephemeral. Host `~/.claude/` is never modified by container activity. Fixes host auth/plugin/MCP config corruption after running `toolbelt claude`.
- `GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE` and `GOOGLE_APPLICATION_CREDENTIALS` now point to writable hydrated copies inside the container instead of read-only `/run/secrets/` paths, fixing potential token-refresh permission errors.
...
- Removed deprecated `container/codex-init-workspace.sh` compatibility stub.
- Removed coordinator section from container MOTD; renamed MOTD title to "Toolbelt Container".
...
- `scripts/verify_toolbelt_opencode_contract.sh` launcher contract verifier for automatic OpenCode import, env override handling, missing-directory failures, and `-kimaki` implication.
- `scripts/verify_toolbelt_opencode_runtime_contract.sh` entrypoint contract verifier for OpenCode runtime hydration, provider file copy-through, invalid JSON warnings, and default `context-mode` preservation.
